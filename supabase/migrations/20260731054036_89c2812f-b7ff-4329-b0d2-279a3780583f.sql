...
USING (bucket_id IN ('avatars','meal-photos') AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "own files write" ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id IN ('avatars','meal-photos') AND (storage.foldername(name))[1] = auth.uid()::text);
